    ...
    <table border="1">
        <tr>
            ...
            <%
                    ...
                    ResultSet rs2 = statement2.executeQuery("Select c.quarter, sum(le.GPA * le.units) / sum(le.units) as gpa from class c, learning le where le.time_period != '2018-SPRING' and le.grade != 'IN' and le.student_SSN = " + request.getParameter("SSN") + " and exists(select s.id, s.class from section s where le.section_id = s.id and s.class = c.id) group by c.quarter");


                    Statement statement3 = conn.createStatement();

                    ResultSet rs3 = statement3.executeQuery("Select sum(le.GPA * le.units) / sum(le.units) as gpa from class c, learning le where le.time_period != '2018-SPRING' and le.grade != 'IN' and le.student_SSN = " + request.getParameter("SSN") + " and exists(select s.id, s.class from section s where le.section_id = s.id and s.class = c.id)");
